...
class DeletePostedJobParams extends Equatable {
  final String jobId;

  const DeletePostedJobParams({required this.jobId});

  @override
  List<Object?> get props => [jobId];
}

class DeletePostedJobUsecase
    implements UsecaseWithParams<void, DeletePostedJobParams> {
  ...
  @override
  Future<Either<Failure, void>> call(DeletePostedJobParams params) async {
    final token = await _tokenSharedPrefs.getToken();
    return token.fold(
      (failure) => Left(failure),
      (tokenValue) =>
          _customerJobsRepository.deletePostedJob(tokenValue, params.jobId),
    );
  }
}
